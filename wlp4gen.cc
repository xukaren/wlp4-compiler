...
void generateCode(shared_ptr<Node> &tree){
    ...
    if (tree->rule == "start BOF procedures EOF"){
    ...
    } else if (tree->rule == "type INT"){
        return;

    } else if (tree->rule == "dcls"){
        return;
        ...
        code(tree->children.at(0)->tokens.at(1));
    ...
    // Q3, Q11: Arithmetic: Integers (including pointers) and operators {+, -, *, /, %} 
        ...
        cout << " ; evaluating = " << type1 << " " << op << " " << type2 << endl; 
        if(op == "PLUS"){       
            generateCode(tree->children.at(0));
        ...
        load(3, tree->children.at(0)->tokens.at(1));
    } 
    ...
    // Q9: Pointers 
    } else if (tree->rule == "type INT STAR"){
        return;
        ...
        if (tree->children.at(1)->rule == "lvalue ID"){
            comment("ID " +tree->children.at(1)->children.at(0)->tokens.at(1));
    ...
    // Q11: Pointer arithmetic (see "Q3: Arithmetic "section)
    
    // Q12: Pointer comparison (see "Q7: Comparison Relations "section)

    // Q13: Procedures with no arguments (other than for wain)
        ...
        // update offsets 
        int num_params = symbolTable[currProc].first.size(); 

        for (VariableTable::iterator it = symbolTable[currProc].second.begin(); it != symbolTable[currProc].second.end(); it++){
            (*it).second.second += num_params*4;
        }
    ...
    // Q14: Procedures with arguments 
        ...
        comment("params paramlist");
        generateCode(tree->children.at(0));


    } else if (tree->rule == "paramlist dcl" || tree->rule == "paramlist dcl COMMA paramlist" ){

        comment("paramlist dcl");

        code(tree->children.at(0)->children.at(1)->tokens.at(1));
        ...
        string proc_name = tree->children.at(0)->tokens.at(1); 
        int num_params = symbolTable[proc_name].first.size(); 
        ...
        cout << " ; NUM PARAMS = " << num_params << endl;
        for (int i = 0;i < num_params; i++){
            pop(5);     // cannot pop back into reg $3 as $3 now stores return value of the procedure just called
        }

        pop(31);
        pop(29);

    } else if (tree->rule == "arglist expr" || tree->rule == "arglist expr COMMA arglist"){
        comment("arg: ");
        generateCode(tree->children.at(0)); 
        push(3);

        if(tree->rule == "arglist expr COMMA arglist"){
            generateCode(tree->children.at(2));
        }
    } 
}
